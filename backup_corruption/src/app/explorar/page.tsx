  ...
  for (const pattern of SECTOR_ICON_PATTERNS) {
    for (const keyword of pattern.keywords) {
      if (normalized.includes(keyword)) {
        if (pattern.iconType === 'emoji') {
          return <span className="text-2xl">{pattern.emoji}</span>;
        } else if (pattern.iconType === 'droplet') {
          return <Droplet className="w-full h-full" />;
        } else if (pattern.iconType === 'waves') {
          return <Waves className="w-full h-full" />;
        } else if (pattern.iconType === 'wind') {
          return <Wind className="w-full h-full" />;
        } else if (pattern.iconType === 'trash') {
